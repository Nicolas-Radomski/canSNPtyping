...
reference = 'Please, site GitHub (https://github.com/Nicolas-Radomski/canSNPtyping) and/or Docker Hub (https://hub.docker.com/r/nicolasradomski/kmerdesignerfast)'

# get parser arguments
parser = ap.ArgumentParser(
	prog='kmerDesigner.py', 
	description='Build Hansel input schema designing kmers harboring canSNPs identified by canSNPextractor.',
	epilog=reference
	)
...
parser.add_argument(
	...
	action='store', 
	required=True, 
	help='path to the chromosome fasta file (REQUIRED)'
	)
parser.add_argument(
	'-a', '--additional', 
	dest='additional', 
...
## add information about the kmer positions
### from files in output directory
for files in os.listdir(path):
	### select files
...
## load whole genome fasta sequence as single line
with open(CHROMOSOME, 'r') as fasta_file:
	### load header (first line)
	header = fasta_file.readline().rstrip()
	### load whole sequence (from second line, first one was removed with .readline())
	seq = ''.join(list(map(lambda x: x.rstrip(), fasta_file.readlines())))

## create upstream and downstream sequence files
## from files in output directory
for files in os.listdir(path):
	## select files
	if files.startswith(PREFIX + '-') and files.endswith('positive-genotypes-random-modif2.tsv'):
		...
		for index, row in random_PG_df.iterrows():
			### create output paths
			outpath_upstream = path + '/' + PREFIX + '-kmer-' + str(index +1) + '.upstream.fasta'
			outpath_downstream = path + '/' + PREFIX + '-kmer-' + str(index +1) + '.downstream.fasta'
			### create start and end parts of header
			h_a = header.split(' ')[0] + ':'
			h_b = ' ' + ' '.join(header.split(' ')[1:])
			### save upstream sequence fasta
			with open(outpath_upstream, 'w') as o:
				#### print header to file
				print(h_a + str(row['Upstream-5']) + '-' + str(row['Upstream-3']) + h_b, file=o)
				#### slice sequence and save to file
				print(seq[row['Upstream-5']-1:row['Upstream-3']], file=o)
			with open(outpath_downstream, 'w') as o:
				#### print header to file
				print(h_a + str(row['Downstream-5']) + '-' + str(row['Downstream-3']) + h_b, file=o)
				#### slice sequence and save to file
				print(seq[row['Downstream-5']-1:row['Downstream-3']], file=o)
